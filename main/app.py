# app.py — Life Minus Work (rich report# app.py — Life Minus Work (rich report with Archetype, Core Need, Metaphor, etc.)
...
for i, q in enumerate(questions, start=1):
    ...
    if sel == WRITE_IN:
        ...
        # IMPORTANT: do not write back to st.session_state[ta_key] after the widget is created;
        # keep the copy only in our dict to avoid StreamlitAPIException.
...
if submit:
    if not email_val or not consent_val:
        ...
        with st.expander("AI status (debug)", expanded=False):
            ...
            st.code(raw_head)
 with Archetype, Core Need, Metaphor, etc.)
from __future__ import annotations
